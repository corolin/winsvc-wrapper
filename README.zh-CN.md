...
- **Session 0 信号投递**：服务没有控制台，rsw 分配一个隐藏控制台并让子进程
  共享。Ctrl 事件由一个短命的辅助进程（rsw 重新拉起自身）附着到子进程的控制台
  后广播，并在自己这一侧吞掉该事件，因此 rsw 不会被自己发出的信号误杀。
  `CREATE_NEW_PROCESS_GROUP` 创建的进程默认禁用 ctrl-c，rsw 会显式重新启用，
  `ctrl_break` 则按进程组精准投递。（`hide_window` 仅为兼容 WinSW 保留，无实际
  作用：子进程始终共享 rsw 的隐藏控制台。）
- **启动进度上报**：每个启动阶段（pre_start 钩子、下载、驱动器映射、拉起子进程）
  都会递增 SCM checkpoint 并给出与该阶段超时匹配的 wait hint，慢钩子或慢下载
  不会触发 1053 启动超时。
